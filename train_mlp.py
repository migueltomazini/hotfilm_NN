...
def show_graphs(data, predictions, train_loss_hist, val_loss_hist):
    ...
    graph_dir = os.path.join(
        data_dir, "train", "results", f"results_{SERIE_IDENTIFIER}", "graphics"
    )
...
def main():
    ...
    if BASE_MODEL_NAME:
        ...
        if os.path.exists(base_scaler_path):
            ...
            print(
                "[Warning] Original base scaler not found! Fine-tuning might produce NaNs."
            )
    ...
    val_loader = DataLoader(
        VoltageVelocityDataset(X_val, Y_val, device), batch_size=best_p["batch_size"]
    )
    ...
    for epoch in range(current_epochs):
        ...
        for X, Y in train_loader:
            ...
            l.backward()
            optimizer.step()
            train_loss_hist.append([idx_t, l.item()])
            idx_t += 1
        ...
        with torch.no_grad():
            for X, Y in val_loader:
                ...
                val_loss_hist.append([idx_v, l_v.item()])
                idx_v += 1

        if epoch % (EPOCHS / 8) == 0:
            print(
                "| Epoch {:4} | train loss {:4.4f} | val loss {:4.4f} |".format(
                    epoch,
                    train_loss_hist[-1][1] if train_loss_hist else 0,
                    val_loss_hist[-1][1] if val_loss_hist else 0,
                ),
                flush=True,
            )
