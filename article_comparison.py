...
RESTORATION & DYNAMIC UPDATES APPLIED:
    - Removed hardcoded physical constants. Parameters are dynamically loaded from config_<serie>.json.
    - U_MEAN is calculated empirically directly from the predicted velocity array.
    - Theoretical references (e.g., Meyers & Meneveau) dynamically plotted based on JSON configuration.
...
def compute_nn_spectrum(signal, u_mean, fs=2000.0):
    """
...
def load_reference_data(serie: str, cfg: dict):
    """
    Dynamically loads reference datasets. Supports legacy 0610 files 
    as well as new theoretical models specified via the JSON config.
    """
    ...
    ts_df, spec_h, spec_s, models_df = None, None, None, None

    if "0610" in serie:
        print("[Info] Series 0610 detected. Loading legacy comparative files...")
        if os.path.exists(ts_path):
            ts_df = pd.read_csv(ts_path, sep=r'\s+', header=None, names=['u1_h', 'u1_s', 'u2_h', 'u2_s', 'u3_h', 'u3_s'])
            ts_df = ts_df.apply(pd.to_numeric, errors='coerce') 
        if os.path.exists(spec_h_path):
            spec_h = pd.read_csv(spec_h_path, sep=r'\s+', header=None, usecols=[0,1,2,3], names=['k1', 'E11', 'E22', 'E33']).dropna()
        if os.path.exists(spec_s_path):
            spec_s = pd.read_csv(spec_s_path, sep=r'\s+', header=None, usecols=[0,1,2,3], names=['k1', 'E11', 'E22', 'E33']).dropna()

    models_path = os.path.join(ext_dir, "models.dat")
    col_e11 = 3
    col_etrans = 4
    
    if "THEORETICAL_MODEL" in cfg and "PATH" in cfg["THEORETICAL_MODEL"]:
        models_path = cfg["THEORETICAL_MODEL"]["PATH"]
        col_e11 = cfg["THEORETICAL_MODEL"].get("E11_COLUMN", 3)
        col_etrans = cfg["THEORETICAL_MODEL"].get("E_TRANS_COLUMN", 4)
        print(f"[Info] Loading configured theoretical model from {models_path}")
        
    if os.path.exists(models_path):
        models_df = pd.read_csv(models_path, sep=r'\s+', header=None, usecols=[0, col_e11, col_etrans], names=['k1', 'E11', 'E_trans'])
...
# PLOTTING FUNCTIONS
# =============================================================================
def plot_figure_4_timeseries(model_df: pd.DataFrame, ts_article: pd.DataFrame, perfect_df: pd.DataFrame, output_dir: str, fs: float):
    print("Generating Figure 4 (Time Series + Zoom Inset)...")
    fig, ax = plt.subplots(figsize=(14, 10))

    min_len = len(model_df)
    if ts_article is not None:
...
def plot_figure_5_spectra(model_df: pd.DataFrame, spec_h: pd.DataFrame, spec_s: pd.DataFrame, models_df: pd.DataFrame, perfect_df: pd.DataFrame, output_dir: str, fs: float, u_mean_global: float, eta: float):
    print("Generating Figure 5 (Energy Spectra)...")
    fig, axes = plt.subplots(1, 3, figsize=(15, 5), sharey=True)
    def k1eta_to_f(x): return (x / eta) * u_mean_global / (2 * np.pi)
    def f_to_k1eta(x): return (x * 2 * np.pi * eta) / u_mean_global
    ...
    for i, (nn_col, perf_col, art_col, title, model_col) in enumerate(components):
        ...
        if spec_s is not None and spec_h is not None:
        ...
        #     ax.loglog(models_df['k1'][mask_mm] * eta, models_df[model_col][mask_mm], color='darkred', linestyle='--', linewidth=2.5, label='Theoretical Model')
            
        if perfect_df is not None:
            k1_perf, E_perf = compute_nn_spectrum(perfect_df[perf_col].values, u_mean=u_mean_global, fs=fs)
            if len(k1_perf) > 0:
                ax.loglog(k1_perf * eta, E_perf, color='#2ecc71', linestyle='-', linewidth=2.0, alpha=0.9, label='True Velocity (Synthetic)')
        
        k1_nn, E_nn = compute_nn_spectrum(model_df[nn_col].values, u_mean=u_mean_global, fs=fs)
        if len(k1_nn) > 0:
...
def plot_figure_6_dissipation(model_df: pd.DataFrame, spec_h: pd.DataFrame, spec_s: pd.DataFrame, models_df: pd.DataFrame, perfect_df: pd.DataFrame, output_dir: str, fs: float, u_mean_global: float, eta: float, norm_factor: float):
    ...
    for i, (nn_col, perf_col, art_col, title, model_col) in enumerate(components):
        ...
        #     ax.plot(models_df.loc[mask_mm, 'k1'] * eta, Diss_mm, color='darkred', linestyle='--', linewidth=2.5, label='Theoretical Model')
...
def plot_diagnostic_raw_dissipation(model_df: pd.DataFrame, perfect_df: pd.DataFrame, output_dir: str, fs: float, u_mean_global: float, eta: float, norm_factor: float):
    """
    DIAGNOSTIC TEST: Plots the RAW (un-magnified) NN prediction against the True Synthetic 
    Velocity to verify if the structural shape of the energy cascade is naturally preserved.
    """
    print("Generating Diagnostic Raw Dissipation Spectra...")
    fig, axes = plt.subplots(1, 3, figsize=(15, 5), sharey=True)
    max_y_global = 0.0
    
    components = [
        ('velocity_raw_predicted_x', 'velocity_x', '(a) Raw Pred $E_{11}$'),
        ('velocity_raw_predicted_y', 'velocity_y', '(b) Raw Pred $E_{22}$'),
        ('velocity_raw_predicted_z', 'velocity_z', '(c) Raw Pred $E_{33}$')
    ]
    
    for i, (nn_col, perf_col, title) in enumerate(components):
        ax = axes[i]
        
        if nn_col in model_df.columns:
            k1_nn, E_nn = compute_nn_spectrum(model_df[nn_col].values, u_mean=u_mean_global, fs=fs)
            if len(k1_nn) > 0:
                Diss_nn = norm_factor * (k1_nn**2) * E_nn
                ax.plot(k1_nn * eta, Diss_nn, color='#e74c3c', linestyle='-', linewidth=2.0, label='Hot-film (Raw Pred)')
                max_y_global = max(max_y_global, np.max(Diss_nn))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "diagnostic_raw_dissipation.png"), dpi=300)
    plt.close()
...
def plot_figure_7_compensated_spectra(
    ...
    for row in range(2):
        ...
        for col, (nn_col, perf_col, art_col, title_top, title_bot, model_col) in enumerate(components):
            ...
            #     ax.semilogx(x_mm, y_mm, color='darkred', linestyle='--', linewidth=2.5, label='Theoretical Model')
...
def main():
    ...
    output_dir = os.path.join(config.DATA_DIR, "run", "results", f"velocity_{serie}", "graphics")
    ...
    if os.path.exists(cfg_path):
        try:
            ...
            print(f"[Info] Falha ao ler o JSON em {cfg_path}: {e}. Usando parâmetros padrão.")
    else:
        print(f"[Info] Arquivo de configuração {cfg_path} não encontrado. Usando parâmetros padrão.")

    fallback_fs = cfg.get("FS_HOTFILM", 2000.0)
    epsilon_val = cfg.get("EPSILON_EXPECTED", 4.88e-3)
    nu_val = cfg.get("KINEMATIC_VISCOSITY", 15.16e-6)
    l_integral_val = cfg.get("L_INTEGRAL", 13.5)
    
    u_mean_global = np.abs(np.nanmean(model_df["velocity_predicted_x"].values))
    print(f"[Info] Empirical longitudinal mean velocity (U_MEAN): {u_mean_global:.4f} m/s")
    print(f"[Info] Config loaded -> EPSILON: {epsilon_val} | NU: {nu_val} | L_INTEGRAL: {l_integral_val}")

    eta_val = (nu_val**3 / epsilon_val)**0.25
    norm_factor_val = (epsilon_val**3 / nu_val)**(-0.25)
    
    fs_real = calculate_real_fs(model_df, time_col="time", fallback_fs=fallback_fs)
    perfect_df = load_perfect_velocity(serie)
    ts_article, spec_h, spec_s, models_df = load_reference_data(serie, cfg)
    
    plot_figure_4_timeseries(model_df, ts_article, perfect_df, output_dir, fs=fs_real)
    plot_figure_5_spectra(model_df, spec_h, spec_s, models_df, perfect_df, output_dir, fs=fs_real, u_mean_global=u_mean_global, eta=eta_val)
    plot_figure_6_dissipation(model_df, spec_h, spec_s, models_df, perfect_df, output_dir, fs=fs_real, u_mean_global=u_mean_global, eta=eta_val, norm_factor=norm_factor_val)
    plot_diagnostic_raw_dissipation(model_df, perfect_df, output_dir, fs=fs_real, u_mean_global=u_mean_global, eta=eta_val, norm_factor=norm_factor_val)
    plot_figure_7_compensated_spectra(model_df, spec_h, spec_s, models_df, perfect_df, output_dir, fs=fs_real, u_mean_global=u_mean_global, eta=eta_val, epsilon=epsilon_val, l_integral=l_integral_val)
    
    print(f"\n✅ All comparison plots (Figures 4, 5, 6, 7 and Diagnostic) successfully saved to: {output_dir}")
