...
parser.add_argument(
    "--block-size", type=int, default=None, help="fixed block length for metrics"
)
parser.add_argument(
    "--gap", type=float, default=None, help="gap threshold (s) for metrics segmentation"
)
...
def validate_synthetic_results(serie, df_predicted, block_size=None, gap=None):
    ...
    if os.path.exists(ref_path):
        ...
        if len(pred_x) != len(ref_x):
            print(
                f"-> Note: Aligning data lengths ({len(pred_x)} vs {len(ref_x)}). Truncating to {min_len} rows."
            )
        ...
        if block_size is not None or gap is not None:
            ...
            merged["ref_x"] = np.concatenate(
                [ref_x, np.zeros(len(df_predicted) - len(ref_x))]
            )[: len(df_predicted)]
            merged["ref_y"] = np.concatenate(
                [ref_y, np.zeros(len(df_predicted) - len(ref_y))]
            )[: len(df_predicted)]
            merged["ref_z"] = np.concatenate(
                [ref_z, np.zeros(len(df_predicted) - len(ref_z))]
            )[: len(df_predicted)]
            ...
            for bi, b in enumerate(blocks):
                ...
                print(
                    f"Block {bi}: RMSE_x={bx_rmse:.6f}, RMSE_y={by_rmse:.6f}, RMSE_z={bz_rmse:.6f}"
                )
...
def export_data_run(df, predictions, destino):
    ...
    print(f"\n\nInput DataFrame:\n\n{df.head()}")
    df_exp.to_csv(
        f"{local_destino}/velocity_{SERIE}.csv", index=False, float_format="%.12f"
    )
...
def runModel():
    ...
    validate_synthetic_results(
        SERIE, df_final, block_size=BLOCK_SIZE, gap=GAP_THRESHOLD
    )
